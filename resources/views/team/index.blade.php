                    ...
                    <x-card class="p-4" x-data="{ editing: false }">
                        <div class="flex items-start gap-3">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <p class="text-xs text-brand-100/60">{{ $member->role ?: 'No role set' }}</p>
                                @unless ($member->photo_path)
                                    <p class="text-[11px] text-amber-200 mt-1">No photo — initials are shown instead.</p>
                                @endunless
                            </div>
                        </div>

                        @if ($member->bio)
                            <p class="mt-3 text-sm text-brand-100/70">{{ $member->bio }}</p>
                        @endif

                        <div class="mt-3 pt-3 border-t border-white/10 flex items-center justify-between">
                            <span class="text-xs text-brand-100/50">Order {{ $member->sort_order }}</span>
