@php
     ...
     */
    $kindTone = [
        ClientCredential::KIND_INSTAGRAM => 'bg-pink-100 text-pink-700',
        ClientCredential::KIND_YOUTUBE => 'bg-red-100 text-red-700',
        ClientCredential::KIND_GOOGLE => 'bg-blue-100 text-blue-700',
        ClientCredential::KIND_FACEBOOK => 'bg-indigo-100 text-indigo-700',
        ClientCredential::KIND_OTHER => 'bg-gray-100 text-gray-600',
    ];
@endphp

<x-card class="p-4 sm:p-6 border border-brand-100/40" x-data="{ adding: {{ $errors->has('kind') || $errors->has('secret') ? 'true' : 'false' }} }">
    <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
            <h3 class="font-semibold text-brand-900">Account logins</h3>
        ...
        <span class="shrink-0 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-gray-100 text-gray-600 text-[10px] font-bold uppercase tracking-wide">
    ...
    <div class="mt-5 space-y-3">
        @forelse ($credentials as $credential)
            <div class="rounded-xl ring-1 ring-gray-900/5 p-4"
                 x-data="{
                     open: false,
                     editing: false,
                     loading: false,
                ...
                <div class="flex flex-wrap items-start justify-between gap-3">
                    <div class="min-w-0">
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            @if ($credential->label)
                                <p class="font-semibold text-gray-900">{{ $credential->label }}</p>
                            @endif
                        </div>

                        <p class="mt-1.5 text-sm text-gray-700 font-mono break-all">{{ $credential->username ?: '—' }}</p>

                        <p class="mt-1 text-xs text-gray-500">
                            @if ($credential->url)
                                <a href="{{ $credential->url }}" target="_blank" rel="noopener noreferrer"
                                   class="text-brand-500 hover:text-brand-600">Open account</a>
                                &middot;
                            @endif
                    ...
                    <div class="shrink-0 flex items-center gap-2">
                        ...
                        <button type="button" @click="editing = ! editing"
                                class="inline-flex items-center min-h-[36px] px-3 rounded-md border border-gray-300 text-gray-700
                                       text-[11px] font-semibold uppercase tracking-wider hover:bg-gray-50 transition">
                            Edit
                        </button>
                    </div>
                </div>

                <p x-show="error" x-cloak x-text="error" class="mt-2 text-xs text-red-700"></p>

                {{-- The revealed value. Selectable so it can be copied, and
                     gone again the moment Hide or Escape is pressed. --}}
                <div x-show="open" x-cloak class="mt-3 rounded-lg bg-amber-50 ring-1 ring-amber-200 p-3">
                    <p class="text-[10px] font-bold uppercase tracking-wide text-amber-800">Password</p>
                    <code class="mt-1 block w-full overflow-x-auto text-sm text-gray-900 select-all" x-text="secret"></code>

                    <template x-if="notes">
                        <div class="mt-3">
                            <p class="text-[10px] font-bold uppercase tracking-wide text-amber-800">Notes</p>
                            <p class="mt-1 text-xs text-gray-700 whitespace-pre-line" x-text="notes"></p>
                        </div>
                    </template>

                    <button type="button" @click="hide()" class="mt-3 text-xs font-semibold text-amber-800 hover:text-amber-900">
                        Hide again
                    </button>
                </div>
                ...
                <form x-show="editing" x-cloak method="POST"
                    ...
                    <div class="flex flex-wrap items-center gap-3">
                        ...
                        <button type="button" @click="editing = false" class="text-sm text-gray-600 hover:text-gray-900">Cancel</button>

                        <button type="submit" form="delete-credential-{{ $credential->id }}"
                                class="ml-auto text-xs font-semibold text-red-600 hover:text-red-700">
                            Delete
                        </button>
                    </div>
                </form>
        ...
        @empty
            <p class="py-6 text-center text-sm text-gray-500">No account logins stored yet.</p>
        @endforelse
    </div>
