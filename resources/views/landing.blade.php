@php
    ...
    */
    ...
    /*
    | Showreel. The Work section is hidden entirely while this is empty, so an
    | empty grid can never be shipped by accident. Each entry:
    |
    |   ['title' => 'Brand film for X', 'kind' => 'YouTube',
    |    'url' => 'https://...', 'thumb' => 'images/work/x.jpg']
    |
    | 'url', 'kind' and 'thumb' are all optional.
    */
    $showreel = [];
    ...
    */
    ...
    $navLinks = ['Services' => '#services', 'Process' => '#process']
        + ($showreel ? ['Work' => '#work'] : [])
        + ['Contact' => '#contact'];
@endphp
...
<head>
    ...
    <title>Chakra Productions &mdash; Video content studio</title>
    <meta name="description" content="Chakra Productions is a video content studio: short-form, YouTube long-form, scripting, editing and publishing, taken from idea to posted.">

    <meta property="og:title" content="Chakra Productions">
    <meta property="og:description" content="A video content studio. Idea to posted.">
...
<body class="antialiased bg-brand-900 text-white" x-data="{ menuOpen: false }">
    ...
    <main id="top">
        ...
        {{-- Work. Hidden entirely while $showreel is empty, so an empty grid
             can never reach a visitor. --}}
        @if ($showreel)
            <section id="work" class="bg-brand-800/40 border-y border-white/10 scroll-mt-20">
                <div class="max-w-6xl mx-auto px-5 sm:px-8 py-20 sm:py-24">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-12">
                        @foreach ($showreel as $piece)
                            {{-- An <a> without href is valid, so one tag covers both cases. --}}
                            <a @if ($piece['url'] ?? false) href="{{ $piece['url'] }}" target="_blank" rel="noopener" @endif
                               class="group block rounded-xl overflow-hidden bg-white/5 border border-white/10 hover:border-brand-400/40 transition-colors">
                                <div class="aspect-video bg-brand-900/60 overflow-hidden">
                                    @if ($piece['thumb'] ?? false)
                                        <img src="{{ asset($piece['thumb']) }}" alt="{{ $piece['title'] }}"
                                             class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300">
                                    @endif
                                </div>
                                <div class="p-5">
                                    @if ($piece['kind'] ?? false)
                                        <p class="text-[11px] font-semibold uppercase tracking-widest text-brand-300 mb-1">{{ $piece['kind'] }}</p>
                                    @endif
                                    <p class="font-semibold">{{ $piece['title'] }}</p>
                                </div>
                            </a>
                        @endforeach
                    </div>
