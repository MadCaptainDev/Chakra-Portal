...
<x-card class="p-4 sm:p-5 border border-brand-100/60 overflow-hidden">
    <div class="flex items-start justify-between gap-3 mb-3">
        <div class="min-w-0">
            <h2 class="font-semibold text-brand-900">Days to decide</h2>
            <p class="text-xs text-gray-500 mt-0.5">
                Accept or send back from here. Open the full timesheet when something needs a closer look.
            </p>
        </div>
        @if ($pendingDays->isNotEmpty())
            <span class="shrink-0 inline-flex items-center min-h-[28px] px-2.5 rounded-md bg-amber-100 text-amber-900 text-[11px] font-semibold tabular-nums">
                {{ $pendingDays->count() }}
            </span>
        @endif
    </div>
    ...
    @if ($pendingDays->isEmpty())
        <p class="text-sm text-gray-500 py-2">You're caught up — no days waiting on a decision.</p>
    @else
        <ul class="divide-y divide-gray-100 -mx-4 sm:-mx-5">
            @foreach ($pendingDays as $item)
                ...
                <li class="px-4 sm:px-5 py-3" x-data="{ open: false, rejecting: false }">
                    <div class="flex flex-wrap items-center gap-3">
                        <button type="button"
                                @click="open = ! open"
                                class="shrink-0 inline-flex items-center justify-center w-8 h-8 rounded-md text-gray-400 hover:bg-brand-50 hover:text-brand-700 transition"
                        ...
                        <div class="min-w-0 flex-1">
                            <p class="text-sm font-semibold text-gray-900 truncate">{{ $item['employee']->name }}</p>
                            <p class="text-[11px] text-gray-500">
                                ...
                                @if ($item['flagged'])
                                    <span class="text-amber-700 font-semibold">&middot; worth a look</span>
                                @endif
                            </p>
                        </div>
                    ...
                    <div id="decide-entries-{{ $rowKey }}" x-show="open" x-cloak class="mt-3 ml-11 space-y-2">
                        @foreach ($item['entries'] as $entry)
                            <div class="rounded-md bg-brand-50/50 ring-1 ring-brand-100/60 px-3 py-2">
                        ...
                        <a href="{{ route('timesheets.show', [$item['employee'], 'month' => $month->format('Y-m')]) }}"
                           class="inline-flex items-center text-xs font-semibold text-brand-600 hover:text-brand-800">
                        ...
                        <form x-show="rejecting" method="POST" action="{{ route('timesheets.day', $item['employee']) }}"
                            ...
                            <div class="flex justify-end gap-2">
                                <button type="button" @click="rejecting = false"
                                        class="inline-flex items-center min-h-[36px] px-3 rounded-md text-xs font-semibold text-gray-600 hover:bg-gray-100">
                                    Cancel
                                </button>
                                <button type="submit"
