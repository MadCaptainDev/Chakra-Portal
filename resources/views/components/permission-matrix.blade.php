...
<div x-data="{ admin: @js($isAdmin) }">
    ...
    <label class="flex items-start gap-3 p-4 rounded-xl ring-1 cursor-pointer transition-colors"
           :class="admin ? 'bg-amber-400/10 ring-amber-400/30' : 'bg-white/5 ring-white/10 hover:bg-white/[0.09]'">
        <input type="checkbox" name="role" value="{{ \App\Models\User::ROLE_ADMIN }}"
               x-model="admin" @checked($isAdmin)
               class="mt-0.5 w-5 h-5 rounded bg-white/10 border-white/25 text-brand-400 focus:ring-brand-400">
        <span class="min-w-0">
            <span class="block text-sm font-semibold text-white">Studio admin</span>
    ...
    <div class="mt-4 flex items-baseline justify-between gap-3">
        ...
        <p class="text-xs text-brand-100/60">Beyond what everyone gets.</p>
    </div>

    {{-- Said out loud, because an empty matrix otherwise reads as "this person
         can do nothing" and invites over-granting. --}}
    <div class="mt-2 rounded-xl bg-brand-900/40 ring-1 ring-white/10 p-4">
        <p class="text-[10px] font-semibold uppercase tracking-[0.14em] text-brand-100/50">Everyone gets</p>
        <ul class="mt-2 space-y-1">
            @foreach (Permission::DEFAULTS as $label => $detail)
                <li class="flex items-start gap-2 text-sm text-brand-100/80">
                    <x-icon name="check-circle" class="w-4 h-4 shrink-0 mt-0.5 text-brand-400" />
                    <span><span class="font-medium">{{ $label }}</span> — <span class="text-brand-100/60">{{ $detail }}</span></span>
                </li>
            @endforeach
            <li class="flex items-start gap-2 text-sm text-brand-100/80">
                <x-icon name="check-circle" class="w-4 h-4 shrink-0 mt-0.5 text-brand-400" />
                <span><span class="font-medium">Team timesheet</span> —
                    <span class="text-brand-100/60">automatically, for anyone named as their manager</span></span>
            </li>
        </ul>
    </div>

    <div class="mt-3 rounded-xl ring-1 ring-white/10 overflow-hidden" :class="admin && 'opacity-50'">
        {{-- Greyed out rather than hidden: hiding it would suggest the account
             has no access, when in fact it has all of it. --}}
        <p x-show="admin" x-cloak class="px-4 py-3 bg-amber-400/10 border-b border-amber-400/30 text-xs text-amber-200">
        ...
        @foreach (Permission::grouped() as $group => $modules)
            <div class="px-4 py-2 bg-brand-900/40 border-b border-white/10">
                <p class="text-[10px] font-semibold uppercase tracking-[0.14em] text-brand-100/50">{{ $group }}</p>
            </div>
            ...
            @foreach ($modules as $module => $config)
                ...
                <div class="p-4 border-b border-white/10 last:border-0">
                    <p class="text-sm font-semibold text-white">{{ $config['label'] }}</p>

                    <div class="mt-2 flex flex-wrap gap-x-5 gap-y-2">
                        @foreach ($config['abilities'] as $ability)
                            <label class="inline-flex items-center gap-2 min-h-[36px] text-sm text-brand-100/80 cursor-pointer"
