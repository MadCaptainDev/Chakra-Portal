<x-app-layout title="My Routines">
    ...
    <div class="max-w-3xl space-y-6">
        @if ($due->isEmpty())
            <x-card padding="sm">
                <p class="text-sm text-gray-500">Nothing due. You are all caught up.</p>
            </x-card>
        @else
            <form method="POST" action="{{ route('my.routines.complete-many') }}" x-data="{ ticked: [] }">
                ...
                <div class="mt-4 flex items-center justify-between gap-3">
                    <p class="text-xs text-gray-500" x-show="ticked.length" x-cloak>
        ...
        @if ($upcoming->isNotEmpty())
            <section>
                <h2 class="text-sm font-semibold uppercase tracking-wider text-gray-500 mb-2">Coming up</h2>
                <x-card padding="sm" class="divide-y divide-gray-100">
                    @foreach ($upcoming as $row)
                        <div class="py-2 first:pt-0 last:pb-0 flex items-baseline justify-between gap-3">
                            <p class="text-sm text-gray-700 min-w-0 truncate">{{ $row['routine']->title }}</p>
